...
```
Отключение синхронной записи (`sync`) увеличит скорость логгирования до 3-х раз, 
но возможны потери последних записей при аварийном завершении. 

Чтобы быстро запретить запись сообщений конкретного лога или типов сообщений всех логов, 
нужно перечислить их в массиве `deny`, например:
```cpp
{
  "path": "/path/to/log/file.log",
  "deny": ["TRACE"]
