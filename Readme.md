...
Для отображения долей секунд используеться опция `resolution` со следующими значенями: `seconds` (без долей), `deciseconds`, `centiseconds`, `milliseconds`, `microseconds` или `nanoseconds`, например:
```cpp
{
  "path": "/path/to/log/file.log",
...
возможно добиться с помощию опции `resolution`, то используем ее.
